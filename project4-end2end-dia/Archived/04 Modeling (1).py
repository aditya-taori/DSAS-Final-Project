...
erc20_token = spark.table("g10_db.erc20_token_transfers")
...
erc20_token.printSchema()

# COMMAND ----------

spark.catalog.isCached("g10_db.erc20_token_transfers")
...
erc20_token.select("token_address").where(col("token_address").isNull()).count()

# COMMAND ----------

tokens_counts = erc20_token.groupby('token_address').count()
#display(tokens_counts)
tokens_counts = tokens_counts.select("*").withColumn("token_ids", monotonically_increasing_id())
#display(tokens_counts)
tokens_counts.printSchema()

# COMMAND ----------

from_address_counts = erc20_token.groupby('from_address').count()
#display(from_address_counts)
from_address_counts = from_address_counts.select("*").withColumn("user_id", monotonically_increasing_id())
#display(from_address_counts)
from_address_counts.printSchema()

# COMMAND ----------

erc20_token = erc20_token.join(from_address_counts,erc20_token.from_address==from_address_counts.from_address)
#display(erc20_token_subset)
erc20_token = erc20_token.join(tokens_counts,erc20_token.token_address==tokens_counts.token_address)
#display(erc20_token_subset)
erc20_token.printSchema()

# COMMAND ----------

triplet_df = (erc20_token.groupBy("user_id","token_ids").count())
...
#predicted_tokens.join(erc20_token, 'user_id') \
#                 .join(erc20_token, 'token_ids') \
